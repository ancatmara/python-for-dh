...
Теперь давайте создадим на компьютере новый текстовый документ с сообщением _Hello world!_ и загрузим его в свой репозиторий. Для этого нужно нажать кнопку _Upload files_ на странице репозитория. 
...
7. Теперь записываем изменения в журнал: `git commit -m "Commit message"`. Сообщение должно коротко и понятно описывать изменения, которые вы вносите -- это поможет вам и вашим коллегам разобраться, что и на каком этапе произошло с проектом. Сообщение писать обязательно! Если вдруг забудете, то попадете в страшное место Vim, из которого очень сложно выбраться! Если вдруг попали -- процедура спасения описана в файле *cmd_cheatsheet* :)
